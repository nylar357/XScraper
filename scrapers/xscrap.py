...
COMMON_SUBDOMAINS = [
    ...
    "intranet", "git", "svn", "docker", "jenkins", "jira", "wiki", "forum"
]
REQUEST_TIMEOUT = 10 # seconds
USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36 ReconScraper/1.0"
# --- End Configuration ---
...
def get_robots_txt(session, url):
    ...
    try:
        ...
        print_error(f"Request timed out while fetching {robots_url}")
    except requests.exceptions.RequestException as e:
        print_error(f"Could not fetch {robots_url}: {e}")
    except Exception as e:
        print_error(f"An unexpected error occurred fetching robots.txt: {e}")


def find_emails(session, url):
    """Scrapes the main page HTML for email addresses."""
    print_header("Emails Found on Homepage")
    try:
        ...
        if emails:
            print_info(f"Found {len(emails)} unique potential email address(es):")
            for email in emails:
                print(f"  - {email}")
        else:
            print_info("No email addresses found on the homepage.")
